...
def accumulate_homographies(H_succesive, m):
    ...
    new_reversed_list = list(accumulate(new_reversed_list, func=lambda x, y: x.dot(y)))
    returned_list[:m] = new_reversed_list[::-1]
...
def compute_bounding_box(homography, w, h):
    ...
    top_left = [np.floor(new_cords[:, 0].min()), np.floor(new_cords[:, 1].min())]
    bottom_right = [np.ceil(new_cords[:, 0].max()), np.ceil(new_cords[:, 1].max())]
...
def warp_channel(image, homography):
    ...
    column = bottom_right[0] - top_left[0]
    returned_image = np.asarray(cur_map_coord).reshape((row, column))
