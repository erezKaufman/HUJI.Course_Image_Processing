...
class PanoramaGenerator:
  ...
  def __init__(self, data_dir, file_prefix, num_images):
    ...
    # Read images.
    self.images = [sol4_utils.read_image(f,1) for f in self.files]
    self.panorama = None
  ...
  def generate_panorama(self):
    ...
    for im in self.images:
      pyramid, _ = sol4_utils.build_gaussian_pyramid(im, 3, 7)
    ...
    images_rgb = [sol4_utils.read_image(f,2) for f in self.files]
